    ...
    <Layout>
      <NextSeo
        ...
        openGraph={{
          ...
          title: 'Portafolio | Andres Coello Goyes',
          description:
            'Mis proyectos mas recientes y publicados en el desarrollo web y movil.',
          images: [
            {
              url: `${BASE_URL}/me.jpg`,
      ...
      <section className="mt-28 mb-12">
        ...
        <p className="text-center mt-2 mb-6">
          Un vistazo a mis trabajos recientes y colaboraciones
        </p>
        <div className="md:p-4 p-2 gap-2 grid sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
          {Portafolio.proyectos.map((proyect: Proyecto) => (
            <CardProyect key={proyect.title} data={proyect} />
          ))}
        </div>
      </section>
    </Layout>
